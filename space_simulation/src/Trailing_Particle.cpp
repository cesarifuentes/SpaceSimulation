...
vector<float> Trailing_Particle::posBuf;
vector<float> Trailing_Particle::colBuf;
vector<float> Trailing_Particle::alpBuf;
vector<float> Trailing_Particle::scaBuf;
GLuint Trailing_Particle::posBufID;
GLuint Trailing_Particle::colBufID;
GLuint Trailing_Particle::alpBufID;
GLuint Trailing_Particle::scaBufID;

// Before this constructor is called, posBuf must be a valid vector<float>.
// I.e., Particle::init(n) must be called first.
Trailing_Particle::Trailing_Particle(int index) :
...
void Trailing_Particle::rebirth(float t, const bool *keyToggles, glm::vec3 drone_position,  glm::vec3 drone_angles, bool areEnginesRunning, bool didTrailDissapate)
{
    m = 1.0f;
    alpha = 0.2f;
    ...
    glm::vec4 b = glm::toMat4(rotation) * glm::vec4(0,0,0.5,0);
    c += Vector3f(b.x, b.y, b.z);
...
void Trailing_Particle::step(float t, float h, const Vector3f &g, const bool *keyToggles, glm::vec3 drone_position, glm::vec3 drone_angles, bool areEnginesRunning, bool didTrailDissapate)
{
    if(t > tEnd && areEnginesRunning) {
        rebirth(t, keyToggles, drone_position, drone_angles, areEnginesRunning, didTrailDissapate);
    }
...
float Trailing_Particle::randFloat(float l, float h)
{
    float r = rand() / (float)RAND_MAX;
    return (1.0f - r) * l + r * h;
}

void Trailing_Particle::init(int n)
{
    posBuf.resize(3*n);
    colBuf.resize(3*n);
    alpBuf.resize(n);
...
void Trailing_Particle::draw(const vector< shared_ptr<Trailing_Particle> > &particles,
